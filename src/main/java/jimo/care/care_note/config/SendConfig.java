...
public class SendConfig implements Consumer<String>, SendFunction {
    ...
    private static Map<String, WeatherIndex> weatherIndexMap;
    private static Map<String, WeatherDay> weatherDayMap;
    ...
    private void cha(Relation relation, Integer power) {
        ...
        if (money > 0 && b) {
            ...
            if (judge(module, power)) {
                ...
                if (weatherIndexMap.get(setting.getLocal()) != null && weatherDayMap.get(setting.getLocal()) != null) {
                    ...
                    if ("ok".equals(msg)) {
                        ...
                        //失败提示运维团队！！！
                        getWeatherError(173, "类中的\"cha()\"方法" + DateUtil.localDateTimeToString(LocalDateTime.now()) + "调用API-weather：" + msg);
                    }
                }
                ...
                if (userService.updateByUID(u1)&&aTrue){
                    ...
                    adminAlter(182,
                            "订单异常","此订单日志记录：’"+aTrue+"‘" +(aTrue?"("+aTrue1+")":"请尽快检查程序！！！")+
                                    ...
                                    "\n异常时间："+DateUtil.localDateTimeToString(LocalDateTime.now()))
                    );
                }
            }else {
    ...
    private boolean judge(CareModule module, Integer power) {
        ...
        if (Objects.equals(power, UserSettingStatus.MORNING)) {
            return auto.equals(module.getMorning());
        } else if (Objects.equals(power, UserSettingStatus.NOON)) {
            return auto.equals(module.getNoon());
        } else if (Objects.equals(power, UserSettingStatus.EVENING)) {
            return auto.equals(module.getEvening());
        }
        return auto.equals(module.getStatus()) || auto.equals(module.getTemp());
    }
    ...
    private void getWeatherError(Integer h, String error) {
        ...
        strings.add("<p style='font-size: 18px;background: linear-gradient(to right, red, blue); -webkit-background-clip: text;color:red;'>Care_NOte运维提醒:API-ERROR</p>");
    ...
    private void adminAlter(Integer h,String alter ,String error) {
        List<String> strings = new ArrayList<>();
        strings.add(String.valueOf(UserPower.ADMIN_ALERT));
        strings.add("<p style='font-size: 18px;background: linear-gradient(to right, red, blue); -webkit-background-clip: text;color:red;'>Care_NOte运维提醒:"+alter+"</p>");
    ...
    private void adminDaily(Integer adminAlert) {
        ...
        stringList.add("<p style='font-size: 18px;background: linear-gradient(to right, red, blue); -webkit-background-clip: text;color:red;'>Care_NOte运维:\"每日详报！\"</p>\"");
    ...
    private void alterUser(User u, String alter,String details){
        List<String> strings = new ArrayList<>();
